...
Refactored by Eric Goldberg, Nov. 22, 2022
"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, groups, downsample, use_bn, drop_out,
        ...
        self.relu1 = nn.ReLU()
        if self.drop_out:
            self.do1 = nn.Dropout(p=self.drop_out)
        ...
        self.relu2 = nn.ReLU()
        if self.drop_out:
            self.do2 = nn.Dropout(p=self.drop_out)
...
class ResNet1D(nn.Module):
    ...
    def __init__(self, in_channels, base_filters, kernel_size, stride, groups, n_block, n_classes, downsample_gap=2,
                 increasefilter_gap=4, use_bn=True, drop_out: Optional[float] = None, verbose=False):
